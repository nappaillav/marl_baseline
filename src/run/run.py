...
def run(_run, _config, _log):
    ...
    # NOTE 2026-08-11: the token MUST include the seed. It previously was "{name}__{timestamp}", so two
    # runs launched in the same second — i.e. any parallel pair — resolved to the SAME tb_logs directory
    # and interleaved their events into one file. Sacred was unaffected (it auto-increments run ids), so
    # only the tensorboard curves were corrupted. Matches marl_project3's convention.
    _task_id = args.env_args.get("map_name", args.env_args.get("env_name", args.env))
    unique_token = "{}_{}_seed{}__{}".format(
        args.name, _task_id, args.seed, datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
