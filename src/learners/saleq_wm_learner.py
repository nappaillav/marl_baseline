...
class SaleqWMLearner(NQLearner):
    """Phase-2 thread C: centralized, mixer-side SEM world model on the SEM base (`phase2-plan_v2.md`).

    Subclass of ``NQLearner`` (guidelines rule 6: never edit ``nq_learner``). The TD path is inherited; this
    learner (a) rebuilds the mixers with the **augmented** state dim ``state_shape + wm_traj_dim`` so they read
    ``[s ; traj]``, and (b) adds the world-model objective ``L_WM = c_kl·L_KL + c_rec·L_rec + c_pred·L_pred +
    c_act·L_act`` (§1). Two passes per batch (§1.6): a teacher-forced model-learning pass (one-step prior, W13
    ``wm_k_train=1``), and an imagination rollout driven by the learned action head that produces ``traj_t``.
    **One rollout from the single live WM feeds BOTH mixers** (sliced live/target — MBVD pattern, no target WM).
    ``L = L_TD + c_wm·L_WM``, single Adam over {agent, mixer, WM}.
    """
    ...
    def __init__(self, mac, scheme, logger, args):
        ...
        # Rebuild the mixers with the augmented state dim (nmix reads args.state_shape at construction; pass a copy).
        aug_args = copy.copy(args)
        aug_args.state_shape = int(np.prod(args.state_shape)) + self.wm.traj_dim
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        aug_state = th.cat([states, traj], dim=-1)                  # [bs, T, state_dim + traj_dim]
